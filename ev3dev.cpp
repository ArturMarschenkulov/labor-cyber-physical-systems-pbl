 ...
 *
 * Modification:
 *  Add new button for ev3dev Release 02.00.00 (ev3dev-jessie-2014-07-12) - Christophe
 * Chaudelet
 *
 */
...
public:
    ...
    V& operator[](const K& k) {
        ...
        if (i != _items.end()) {
            ...
            while (_items.size() + 1 > _size) { _items.pop_back(); }
...
int device::device_index() const {
    ...
    if (_path.empty()) {
        throw system_error(
            make_error_code(errc::function_not_supported), "no device connected"
        );
    }
...
std::string device::get_attr_from_set(const std::string& name) const {
    ...
    do {
        ...
        if (!t.empty()) {
            if (*t.begin() == '[') { return t.substr(1, t.length() - 2); }
        }
    } while (pos != string::npos);
...
bool sensor::connect(const std::map<std::string, std::set<std::string>>& match) noexcept {
    ...
    try {
        if (device::connect(_strClassDir, _strPattern, match)) { return true; }
    } catch (...) {}
...
std::string sensor::type_name() const {
    ...
    if (s != lookup_table.end()) { return s->second; }
...
int sensor::value(unsigned index) const {
    if (static_cast<int>(index) >= num_values()) { throw std::invalid_argument("index"); }
...
const std::vector<char>& sensor::bin_data() const {
    ...
    if (_bin_data.empty()) {
        ...
        if (s != lookup_table.end()) { value_size = s->second; }
...
//-----------------------------------------------------------------------------
void led::all_off() { amber_led1.off(); }
...
std::vector<float> led::black{
    static_cast<float>(0),
    static_cast<float>(0),
    static_cast<float>(0)
};
std::vector<float> led::red{
    static_cast<float>(1),
    static_cast<float>(0),
    static_cast<float>(0)
};
std::vector<float> led::green{
    static_cast<float>(0),
    static_cast<float>(1),
    static_cast<float>(0)
};
std::vector<float> led::blue{
    static_cast<float>(0),
    static_cast<float>(0),
    static_cast<float>(1)
};
std::vector<float> led::yellow{
    static_cast<float>(1),
    static_cast<float>(1),
    static_cast<float>(0)
};
std::vector<float> led::purple{
    static_cast<float>(1),
    static_cast<float>(0),
    static_cast<float>(1)
};
std::vector<float> led::cyan{
    static_cast<float>(0),
    static_cast<float>(1),
    static_cast<float>(1)
};
std::vector<float> led::white{
    static_cast<float>(1),
    static_cast<float>(1),
    static_cast<float>(1)
};
std::vector<float> led::orange{
    static_cast<float>(1),
...
void led::set_color(const std::vector<led*>& group, const std::vector<float>& color) {
    ...
    for (size_t i = 0; i < n; ++i) { group[i]->set_brightness_pct(color[i]); }
}
...
power_supply::power_supply(std::string name) {
    ...
    if (name.empty()) { name = "legoev3-battery"; }
...
void lcd::fill(unsigned char pixel) {
    if (_fb && _fbsize) { memset(_fb, pixel, _fbsize); }
}
...
void lcd::init() {
    ...
    fb_fix_screeninfo i;
    if (ioctl(fbf, FBIOGET_FSCREENINFO, &i) < 0) { return; }
    ...
    if (ioctl(fbf, FBIOGET_VSCREENINFO, &v) < 0) { return; }
...
remote_control::remote_control(unsigned channel)
    ...
    , _owns_sensor(true) {
    if ((channel >= 1) && (channel <= 4)) { _channel = channel - 1; }

    if (_sensor->connected()) { _sensor->set_mode(infrared_sensor::mode_ir_remote); }
}
...
remote_control::~remote_control() {
    if (_owns_sensor) { delete _sensor; }
}
...
void remote_control::on_value_changed(int value) {
    ...
    switch (value) {
        case 1: new_state = red_up; break;
        case 2: new_state = red_down; break;
        case 3: new_state = blue_up; break;
        case 4: new_state = blue_down; break;
        case 5: new_state = red_up | blue_up; break;
        case 6: new_state = red_up | blue_down; break;
        case 7: new_state = red_down | blue_up; break;
        case 8: new_state = red_down | blue_down; break;
        case 9: new_state = beacon; break;
        case 10: new_state = red_up | red_down; break;
        case 11: new_state = blue_up | blue_down; break;
    }
...
bool lego_port::connect(const std::map<std::string, std::set<std::string>>& match
) noexcept {
